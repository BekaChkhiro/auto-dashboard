  ...
  try {
    ...
    if (!rateLimit.allowed) {
      return NextResponse.json(
        {
          error: `Too many upload requests. Please try again in ${rateLimit.retryAfterSeconds} seconds.`,
        },
        {
          status: 429,
          headers: {
    ...
    if (context.type === 'receipt') {
      ...
      if (user.role !== 'DEALER' && user.role !== 'ADMIN') {
        return NextResponse.json({ error: 'Only dealers can upload receipts' }, { status: 403 })
      }
    }
